...
type ConceptContract = Readonly<{
  sessions: ReadonlyArray<string>;
  ownedConcepts: ReadonlyArray<string>;
  ownedIdentifierPaths: ReadonlyArray<string>;
  publicApiPaths: ReadonlyArray<string>;
  requiredPublicExports: ReadonlyArray<PublicExportRequirement>;
}>;
...
const moonbaseContract: ConceptContract = {
  ...
  ownedConcepts: ["permit", "lockout", "worker", "spaceWeather"],
  ownedIdentifierPaths: [
    "domain/permit/permitId.ts",
    "domain/permit/zoneId.ts",
    "domain/lockout/segmentId.ts",
    "domain/worker/workerId.ts",
  ],
  publicApiPaths: [
    "domain/permit/index.ts",
    "domain/lockout/index.ts",
    "domain/worker/index.ts",
    "domain/spaceWeather/index.ts",
  ],
  requiredPublicExports: [
    { concept: "permit", module: "permitApi", symbol: "EvaPermit", typeOnly: true, sessions: ["07", "08"] },
    { concept: "permit", module: "permitApi", symbol: "EvaPermit", sessions: ["07", "08"] },
    { concept: "permit", module: "permit", symbol: "EvaPermit", typeOnly: true, sessions: ["04", "05", "06"] },
    { concept: "permit", module: "permitId", symbol: "PermitId" },
    { concept: "permit", module: "zoneId", symbol: "ZoneId" },
    { concept: "permit", module: "statusLabel", symbol: "toStatusLabel" },
    { concept: "permit", module: "transitions", symbol: "approve" },
    { concept: "permit", module: "transitions", symbol: "egress" },
    { concept: "permit", module: "transitions", symbol: "returnToBase" },
    { concept: "permit", module: "transitions", symbol: "close" },
    { concept: "permit", module: "transitions", symbol: "abort" },
...
const isSnapshotPackageImport = (specifier: string): boolean =>
  /^@moonbase\/session-\d{2}(?:\/|$)/.test(specifier);
...
const conceptForFile = (
  file: URL,
  sessionUrl: URL,
  concepts: ReadonlyArray<string> = finalPublicApis,
): string | undefined =>
...
const writeFinalPublicApiFixture = async (
  ...
  for (const concept of finalPublicApis) {
    ...
    if (options.ownIndexImport && concept === "permit") {
      await writeFile(
    ...
    if (options.localExport && concept === "permit") {
...
describe("runnable session package contract", () => {
  it("keeps local SQLite databases and journals outside Git", () => {
    const databasePaths = [
      "examples/session-00/moonbase.sqlite",
      "examples/session-02/moonbase.sqlite",
      "examples/session-03/moonbase.sqlite",
      "examples/session-04/moonbase.sqlite",
      "examples/session-05/moonbase.sqlite",
      "examples/session-05/moonbase.sqlite-journal",
      "examples/session-05/moonbase.sqlite-wal",
      "examples/session-05/moonbase.sqlite-shm",
      "examples/session-06/moonbase.sqlite",
      "examples/session-07/moonbase.sqlite",
      "examples/session-08/moonbase.sqlite",
      "examples/session-08/moonbase.sqlite-journal",
      "examples/session-08/moonbase.sqlite-wal",
      "examples/session-08/moonbase.sqlite-shm",
    ];
  ...
  it("discovers all nine sessions with runnable package scripts", async () => {
    ...
    for (const directory of sessionDirectories) {
      ...
      expect(packageJson.name, directory).toBe(`@moonbase/${directory}`);
  ...
  it("connects every demo and shared Web package to root commands", async () => {
    ...
    for (const session of ["00", "01", "02", "03", "04", "05", "06", "07", "08"]) {
    ...
    for (const command of ["build", "test", "typecheck"]) {
      ...
      expect(packageJson.scripts?.[command], command).toContain("@moonbase/base-web");
    }
    expect(packageJson.scripts).toHaveProperty(
      "test:continuity",
      "pnpm --filter @moonbase/approve-eva-continuity test",
    );
    expect(packageJson.scripts).toHaveProperty(
      "typecheck:continuity",
      "pnpm --filter @moonbase/approve-eva-continuity typecheck",
    );
  ...
  it("keeps session identifiers in their owning domain concepts", async () => {
    ...
    for (const session of contract.sessions) {
      ...
      for (const path of [...contract.ownedIdentifierPaths, ...contract.publicApiPaths]) {
      ...
      for (const file of files) {
        ...
        const fileConcept = conceptForFile(file, sessionUrl, contract.ownedConcepts);
        for (const specifier of relativeJavaScriptImports(source)) {
          const target = resolvesRelativeJavaScriptImport(specifier, file);
          const targetConcept = conceptForFile(target, sessionUrl, contract.ownedConcepts);
          if (targetConcept === undefined) continue;
      ...
      for (const concept of contract.ownedConcepts) {
        ...
        for (const requirement of contract.requiredPublicExports) {
