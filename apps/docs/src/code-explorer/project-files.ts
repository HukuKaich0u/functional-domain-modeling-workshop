...
import moonbaseFixtureSource from "../../../../examples/fixtures/moonbase.ts?raw";
import type { ExampleSnapshot } from "../sessions/types";
import type { ProjectFiles } from "./types";

type ProjectSnapshot = Exclude<ExampleSnapshot, "session-02">;
...
const buildProjectFiles = (snapshot: ProjectSnapshot): ProjectFiles => {
  ...
  const exerciseSequence = /^session-(0[3-7])$/.exec(snapshot)?.[1];

  return Object.freeze({
    ...files,
    "../fixtures/clinic.ts": clinicFixtureSource,
    "../fixtures/moonbase.ts": moonbaseFixtureSource,
