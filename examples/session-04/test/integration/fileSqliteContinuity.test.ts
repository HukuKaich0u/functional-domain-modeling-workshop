...
test("file SQLite reopens the Session 04 approved state and leaking work log context", async () => {
  const directory = mkdtempSync(join(tmpdir(), "session-04-"));
...
test("Session 04 does not add a work log when a Closed permit is approved again", async () => {
  const directory = mkdtempSync(join(tmpdir(), "session-04-closed-"));
  directories.push(directory);
  const options = {
    databasePath: join(directory, "moonbase.sqlite"),
    migrationsFolder: fileURLToPath(new URL("../../drizzle", import.meta.url)),
  ...
  try {
    ...
    let beforeWorkLogCount!: number;
    try {
      const before = beforeDatabase
        .prepare("SELECT state FROM permits WHERE permit_id = ?")
        .get(moonbaseFixture.permitId) as Readonly<{ state: string }>;
      beforeState = JSON.parse(before.state) as unknown;
      expect(beforeState).toMatchObject({ kind: "Closed" });
      beforeWorkLogCount = (beforeDatabase
    ...
    try {
      const after = afterDatabase
        .prepare("SELECT state FROM permits WHERE permit_id = ?")
        .get(moonbaseFixture.permitId) as Readonly<{ state: string }>;
      const afterWorkLogCount = afterDatabase
        .prepare("SELECT count(*) AS count FROM work_logs")
        .get() as Readonly<{ count: number }>;

      expect(JSON.parse(after.state)).toEqual(beforeState);
      expect(afterWorkLogCount.count).toBe(beforeWorkLogCount);
    } finally {
      afterDatabase.close();
    }
  } finally {
