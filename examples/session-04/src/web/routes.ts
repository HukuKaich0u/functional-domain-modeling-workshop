...
export const session04InitialPermit: Requested = {
  kind: "Requested",
...
const fixtureEquipmentChecks = [
  {
    workerId: WorkerId.parse(moonbaseFixture.crew[0]),
    oxygenMinutes: moonbaseFixture.oxygenMinutes,
    checkedAt: moonbaseFixture.checkedAt,
  },
  {
    workerId: WorkerId.parse(moonbaseFixture.crew[1]),
    oxygenMinutes: moonbaseFixture.oxygenMinutes,
    checkedAt: moonbaseFixture.checkedAt,
  },
] as const;
...
export const registerMoonbaseRoutes = (
  ...
  app.post("/demo/reset", (context) => {
    repository.reset(session04InitialPermit, session04PersistenceContext);
    return context.redirect("/", 303);
  });
};
