...
import { createDatabaseBackedApp as createSession08App } from "../../session-08/src/app.js";
import type { Clock } from "../../session-08/src/domain/aggregate/clock.js";
import type { EventIdGenerator } from "../../session-08/src/domain/aggregate/eventIdGenerator.js";
...
export type SnapshotApp = Readonly<{
...
const stateFromKind = (state: unknown): SnapshotState => {
  if (typeof state !== "object" || state === null) {
    throw new Error("Persisted permit state must be an object");
  }

  const kind = (state as Readonly<{ kind?: unknown }>).kind;
  if (typeof kind !== "string" || !snapshotStates.includes(kind as SnapshotState)) {
    throw new Error("Persisted permit state must contain a supported kind");
  }
...
const stateFromStatus = (state: unknown): SnapshotState => {
  ...
  if (typeof status !== "string" || statesByStatus[status] === undefined) {
    throw new Error("Persisted permit state must contain a supported status");
  }
...
export const snapshotScenarios: readonly SnapshotScenario[] = [
  ...
  createScenario("Session 02", createSession02App, migrationFolderFor("02"), stateFromStatus),
  ...
  createScenario("Session 07", createSession07App, migrationFolderFor("07"), stateFromKind),
  createScenario(
    "Session 08",
    createSession08App,
    migrationFolderFor("08"),
    stateFromKind,
    createSession08App,
  ),
];
