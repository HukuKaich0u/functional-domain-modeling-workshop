...
import { parseBody, parseIdentifier } from "../middleware/requestBody.js";
...
/** 管理画面向け。Sensitive はここで明示的に unwrap する */
...
const parseUserId = (context: Context<WebEnvironment>, raw: string) =>
  parseIdentifier(context, UserId.schema, raw);
...
const userIndexErrors = (rawError: string | undefined): FieldErrors => {
  ...
  switch (code) {
    ...
    case "cannot-delete-last-admin":
      return { form: "最後の Admin は削除できません。" };
    default:
...
export const registerUserRoutes = (
  ...
  app.post("/users", async (context) => {
    ...
    const parsed = await parseBody(context, CreateUserFormSchema);
  ...
  app.post("/users/:userId", async (context) => {
    ...
    const parsed = await parseBody(context, UpdateUserFormSchema);
    if (parsed.isErr()) {
      return renderEdit(
        context,
        authorized.value,
  ...
  app.post("/users/:userId/reset-password", async (context) => {
    ...
    const userId = parseUserId(context, context.req.param("userId"));
    if (userId.isErr()) return userId.error;
    const parsed = await parseBody(context, ResetPasswordFormSchema);
    if (parsed.isErr()) {
      return renderEdit(
        context,
        authorized.value,
