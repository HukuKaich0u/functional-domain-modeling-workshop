...
export default function UserForm({ auth, errors, mode, user }: UserFormProps) {
  ...
  const profileFields = (
    <>
      <FormField {...(errors.name === undefined ? {} : { error: errors.name })} field="name" label="表示名">
        <input
          autoComplete="name"
      ...
      <FormField {...(errors.role === undefined ? {} : { error: errors.role })} field="role" label="役割">
        ...
        >
          <option value="Admin">Admin（システム担当）</option>
          <option value="GroundControl">GroundControl（地上管制）</option>
          <option value="BaseCommander">BaseCommander（基地長）</option>
