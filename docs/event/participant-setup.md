...
コードを編集する演習は S3〜S7 の5本です。S2 は「予約がキャンセルされた」からアクター、コマンド、確認する条件を逆算し、Excalidrawで図にする回です。ひとりでも複数人でも進められます。コマンドは実行しません。
...
ひとりの場合は、S3〜S6の各ページの問いに答え、自分の差分から根拠となるコードを挙げます。型検査では確認できず、テストや実行時に確認することを [レビュー観点シート](./review-sheet.md) に記録してください。
